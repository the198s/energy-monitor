...
function SliderInput({ label, value, min, max, step, unit, onChange }) {
  ...
  const inputId = `slider-${label.replace(/\W+/g, "-").toLowerCase()}`;
  return (
    <div style={{ marginBottom:20 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:8 }}>
        <label htmlFor={inputId} style={{ fontSize:13, fontWeight:600, color:"#374151", fontFamily:"'Inter',sans-serif" }}>{label}</label>
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          <input id={inputId} type="number" value={value} min={min} max={max} step={step}
            onChange={e => onChange(Math.min(max, Math.max(min, Number(e.target.value))))}
            style={{ width:90, padding:"4px 8px", border:"1.5px solid #e5e7eb", borderRadius:8, fontSize:15, fontWeight:700, color:"#111827", fontFamily:"'Inter',sans-serif", textAlign:"right" }} />
          <span style={{ fontSize:13, color:"#6b7280", whiteSpace:"nowrap" }}>{unit}</span>
        </div>
      </div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }} aria-hidden="true">
        <span style={{ fontSize:12, color:"#9ca3af" }}>{min.toLocaleString()}</span>
        <span style={{ fontSize:12, color:"#9ca3af" }}>{max.toLocaleString()}</span>
      </div>
    </div>
  );
}
...
function RateField({ label, value, onChange, unit }) {
  const inputId = `rate-${label.replace(/\W+/g, "-").toLowerCase()}`;
  return (
    <div style={{ background:"#f8fafc", border:"1.5px solid #e5e7eb", borderRadius:10, padding:"10px 12px" }}>
      <label htmlFor={inputId} style={{ display:"block", fontSize:11, color:"#6b7280", marginBottom:6 }}>{label}</label>
      <div style={{ display:"flex", alignItems:"center", gap:4 }}>
        <input id={inputId} type="number" value={value} step="0.001" min={0}
          onChange={e => onChange(Number(e.target.value))}
          style={{ flex:1, border:"none", background:"transparent", fontSize:18, fontWeight:700, color:"#111827", fontFamily:"'Inter',sans-serif", minWidth:0 }} />
...
function DealCard({ deal, rank, currentTotal }) {
  ...
  return (
    <div className="deal-card" style={{ background:"#fff", border:`1.5px solid ${beats?"#86efac":beatsEon?"#bfdbfe":"#e5e7eb"}`, borderRadius:14, overflow:"hidden", boxShadow:beats?"0 4px 24px rgba(22,163,74,0.08)":"0 1px 4px rgba(0,0,0,0.04)", marginBottom:10 }}>
      {beats && <div style={{ background:"linear-gradient(90deg,#16a34a,#22c55e)", padding:"5px 16px", fontSize:12, color:"#fff", fontWeight:700 }}>✓ Beats your current deal</div>}
      <div style={{ padding:"16px 18px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
              <span style={{ background:beats?"#f0fdf4":beatsEon?"#eff6ff":"#f9fafb", color:beats?"#15803d":beatsEon?"#1d4ed8":"#6b7280", border:`1px solid ${beats?"#bbf7d0":beatsEon?"#bfdbfe":"#e5e7eb"}`, borderRadius:20, fontSize:12, fontWeight:700, padding:"2px 8px" }}>
                #{rank}{beats?" · Best":beatsEon?" · Good":""}
              </span>
              <span style={{ fontSize:12, color:confColor, fontWeight:600 }}>{capitalize(deal.confidence)} confidence</span>
            </div>
            <div style={{ fontSize:13, color:"#6b7280" }}>{deal.supplier}</div>
            <div style={{ fontSize:16, fontWeight:700, color:"#111827" }}>{deal.tariffName}</div>
            <div style={{ fontSize:13, color:"#6b7280", marginTop:2 }}>{deal.term} · Exit £{deal.exitFee}</div>
          </div>
        ...
        <button onClick={()=>setOpen(v=>!v)} aria-expanded={open} style={{ background:"none", border:"none", padding:"8px 0", fontSize:13, fontWeight:600, color:"#2563eb", cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:4, minHeight:44 }}>
            ...
            {[
              ...
              return (
                <div key={l} style={{ background:better?"#f0fdf4":"#fafafa", border:`1px solid ${better?"#bbf7d0":"#f3f4f6"}`, borderRadius:8, padding:"8px 10px" }}>
                  ...
                  <div style={{ fontSize:14, fontWeight:600, color:better?"#15803d":"#374151" }}>{v}{unit} {better?"▼":"▲"}</div>
                  <div style={{ fontSize:11, color:"#6b7280" }}>yours: {ref}{unit}</div>
                </div>
              );
            })}
            {deal.notes && <div style={{ gridColumn:"1/-1", fontSize:12, color:"#6b7280", background:"#fafafa", borderRadius:6, padding:"6px 8px" }}>📝 {deal.notes}</div>}
            <div style={{ gridColumn:"1/-1", fontSize:12, color:"#6b7280" }}>Source: {deal.source}</div>
          </div>
        )}
      </div>
    </div>
...
export default function Home() {
  ...
  const runAgent = async () => {
    ...
    try {
      const res = await fetch("/api/search", {
        ...
        body: JSON.stringify({ usage, tariff, dealType }),
      });
  ...
  const Label = ({children, mb=14}) => (
    <div style={{ fontSize:12, color:"#6b7280", fontWeight:600, marginBottom:mb }}>{children}</div>
  );

  const EditBtn = ({onClick, label}) => (
    <button onClick={onClick} aria-label={label} style={{ background:"#f1f5f9", border:"1px solid #e5e7eb", borderRadius:8, padding:"5px 12px", fontSize:13, fontWeight:600, color:"#374151", cursor:"pointer", fontFamily:"'Inter',sans-serif", flexShrink:0 }}>✏️ Edit</button>
  );

  const SaveCancel = ({onSave, onCancel, label="Save"}) => (
    <div style={{ display:"flex", gap:8, marginTop:16 }}>
      <button onClick={onSave} style={{ flex:1, padding:"11px", background:"linear-gradient(135deg,#2563eb,#7c3aed)", border:"none", borderRadius:10, color:"#fff", fontSize:14, fontWeight:700, fontFamily:"'Inter',sans-serif", cursor:"pointer" }}>{label}</button>
      <button onClick={onCancel} style={{ padding:"11px 18px", background:"#f8fafc", border:"1px solid #e5e7eb", borderRadius:10, color:"#6b7280", fontSize:14, fontFamily:"'Inter',sans-serif", cursor:"pointer" }}>Cancel</button>
    </div>
  );

  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#eef2ff 0%,#f8fafc 28%)", fontFamily:"'Inter',sans-serif" }}>

      {/* Nav */}
      <nav aria-label="Main navigation" style={{ background:"rgba(255,255,255,0.85)", backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", borderBottom:"1px solid #e5e7eb", padding:"0 24px", height:56, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10, boxShadow:"0 1px 8px rgba(0,0,0,0.06)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <span style={{ fontWeight:700, fontSize:16, letterSpacing:"-0.01em" }}>Energy Monitor</span>
        </div>
        {lastRun && <span style={{ fontSize:13, color:"#6b7280" }}>Last checked {lastRun.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}</span>}
      </nav>
      ...
      <main style={{ maxWidth:680, margin:"0 auto", padding:"28px 18px 60px" }}>
        ...
        <div style={{ marginBottom:28 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:99, padding:"4px 12px", marginBottom:14 }}>
            ...
            <span style={{ fontSize:13, fontWeight:600, color:"#1d4ed8" }}>UK energy market · live search</span>
          </div>
          ...
          <p style={{ fontSize:14, color:"#6b7280", lineHeight:1.7 }}>Searches MSE, Uswitch and supplier sites · Calculates costs against your exact usage</p>
        </div>

        {/* ── Usage ── */}
        <Card mb={12}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: editingUsage ? 20 : 14 }}>
            <Label mb={0}>Your annual usage</Label>
            {!editingUsage && <EditBtn onClick={()=>{setDraftUsage(usage);setEditingUsage(true)}} label="Edit annual usage" />}
          </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {[{icon:"⚡",label:"Electricity",val:`${usage.electricityKwh.toLocaleString()} kWh/yr`},{icon:"🔥",label:"Gas",val:`${usage.gasKwh.toLocaleString()} kWh/yr`}].map(({icon,label,val})=>(
                <div key={label} style={{ background:"#f8fafc", border:"1px solid #f1f5f9", borderRadius:10, padding:"12px 14px" }}>
                  ...
                  <div style={{ fontSize:18, fontWeight:800, color:"#111827", marginTop:2 }}>{val}</div>
                </div>
              ))}
            </div>
          ) : (
        ...
        <Card mb={20}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            <Label mb={0}>Your current tariff</Label>
            {!editingTariff && <EditBtn onClick={()=>{setDraftTariff(tariff);setEditingTariff(true)}} label="Edit current tariff" />}
          </div>

          {!editingTariff ? (
            <>
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:13, color:"#6b7280" }}>{tariff.supplier}</div>
                <div style={{ fontSize:17, fontWeight:700, color:"#111827" }}>{tariff.tariffName}</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:14 }}>
                {[
                  ...
                  <div key={l} style={{ background:"#f8fafc", border:"1px solid #f1f5f9", borderRadius:8, padding:"8px 10px" }}>
                    <div style={{ fontSize:11, color:"#6b7280", marginBottom:2 }}>{l}</div>
                    <div style={{ fontSize:15, fontWeight:700, color:"#1d4ed8" }}>{v}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:10, padding:"12px 16px" }}>
                <div>
                  <div style={{ fontSize:12, color:"#1d4ed8", fontWeight:600, marginBottom:3 }}>Estimated annual cost</div>
                  <div style={{ fontSize:13, color:"#1d4ed8", marginTop:2 }}>⚡ £{cost.elec.toLocaleString()} · 🔥 £{cost.gas.toLocaleString()}</div>
                </div>
                <div style={{ fontSize:30, fontWeight:800, color:"#1d4ed8" }}>£{cost.total.toLocaleString()}</div>
              </div>
            </>
          ) : (
            <>
              <div style={{ marginBottom:14 }}>
                ...
                <select value={draftTariff.supplier} onChange={e=>setDT("supplier",e.target.value)}
                  style={{ width:"100%", padding:"10px 12px", border:"1.5px solid #e5e7eb", borderRadius:8, fontSize:14, color:"#111827", fontFamily:"'Inter',sans-serif", background:"#fff" }}>
                  {UK_SUPPLIERS.map(s=><option key={s} value={s}>{s}</option>)}
                </select>
              </div>

              <div style={{ marginBottom:14 }}>
                <label style={{ fontSize:13, fontWeight:600, color:"#374151", display:"block", marginBottom:6 }}>Tariff name</label>
                <input type="text" value={draftTariff.tariffName} onChange={e=>setDT("tariffName",e.target.value)}
                  ...
                  style={{ width:"100%", padding:"10px 12px", border:"1.5px solid #e5e7eb", borderRadius:8, fontSize:14, color:"#111827", fontFamily:"'Inter',sans-serif" }} />
              </div>

              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
                <div>
                  <label style={{ fontSize:13, fontWeight:600, color:"#374151", display:"block", marginBottom:6 }}>Contract length</label>
                  <input type="text" value={draftTariff.term} onChange={e=>setDT("term",e.target.value)} placeholder="e.g. 14 months"
                    style={{ width:"100%", padding:"10px 12px", border:"1.5px solid #e5e7eb", borderRadius:8, fontSize:14, color:"#111827", fontFamily:"'Inter',sans-serif" }} />
                </div>
                <div>
                  <label style={{ fontSize:13, fontWeight:600, color:"#374151", display:"block", marginBottom:6 }}>Deal end date</label>
              ...
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#374151", marginBottom:10 }}>⚡ Electricity rates</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              ...
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#374151", marginBottom:10 }}>🔥 Gas rates</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              ...
              <div style={{ marginBottom:16 }}>
                <label style={{ fontSize:13, fontWeight:600, color:"#374151", display:"block", marginBottom:6 }}>Early exit fee (£ total, both fuels)</label>
              ...
              {(()=>{
                ...
                return (
                  <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:10, padding:"10px 16px", marginBottom:4, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div style={{ fontSize:13, color:"#15803d" }}>Live preview · ⚡ £{p.elec} · 🔥 £{p.gas}</div>
                    <div style={{ fontSize:20, fontWeight:800, color:"#15803d" }}>£{p.total}/yr</div>
                  </div>
                );
              })()}
        ...
        <Card mb={16}>
          ...
          <div style={{ display:"flex", gap:2, background:"#f1f5f9", borderRadius:10, padding:3 }}>
            {[
              ...
              <button key={key} onClick={() => setDealType(key)} disabled={running}
                style={{ flex:1, padding:"8px 10px", background:dealType===key?"#fff":"transparent", border:"none", borderRadius:7, color:dealType===key?"#111827":"#6b7280", fontSize:14, fontWeight:dealType===key?700:500, cursor:running?"not-allowed":"pointer", transition:"all 0.15s", boxShadow:dealType===key?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:24 }}>
          <button onClick={runAgent} disabled={running} style={{ flex:1, padding:"14px 20px", background:running?"#f1f5f9":"linear-gradient(135deg,#2563eb,#7c3aed)", border:"none", borderRadius:12, color:running?"#9ca3af":"#fff", fontSize:15, fontWeight:700, fontFamily:"'Inter',sans-serif", cursor:running?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, boxShadow:running?"none":"0 4px 16px rgba(37,99,235,0.3)", transition:"all 0.2s" }}>
          ...
          {running && (
            <button onClick={cancel} aria-label="Cancel search" style={{ padding:"14px 18px", borderRadius:12, background:"#fff", border:"1.5px solid #e5e7eb", color:"#dc2626", fontSize:14, fontWeight:700, fontFamily:"'Inter',sans-serif", cursor:"pointer", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>✕ Cancel</button>
          )}
        </div>
          ...
          <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:16, padding:"22px 22px 20px", marginBottom:20, boxShadow:"0 4px 20px rgba(37,99,235,0.08)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:20 }}>
              <div style={{ position:"relative", flexShrink:0 }}>
                ...
                <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:13, color:"#2563eb" }}>{progress}%</div>
              </div>
              <div>
                <div style={{ fontWeight:700, fontSize:16, color:"#111827", marginBottom:2 }}>{currentStep?.label ?? "Starting up…"}</div>
                <div style={{ fontSize:13, color:"#6b7280" }}>Searching energy comparison sites in real time</div>
              </div>
            </div>
            ...
            <div role="list" aria-live="polite" aria-label="Search progress" style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {STEPS.map(step=>{
                ...
                return (
                  <div key={step.id} style={{ display:"flex", alignItems:"center", gap:10, opacity:done||active?1:0.3, transition:"opacity 0.4s" }}>
                    ...
                    <span style={{ fontSize:13, color:done?"#16a34a":active?"#2563eb":"#6b7280", fontWeight:active?600:400 }}>{step.label}</span>
                    {active&&<span style={{fontSize:12,color:"#6b7280",marginLeft:"auto",animation:"pulse 2s ease infinite"}}>in progress…</span>}
                    {done&&<span style={{fontSize:12,color:"#16a34a",marginLeft:"auto"}}>done</span>}
                  </div>
                );
              })}
            </div>
        ...
        {status==="cancelled" && (
          <div role="alert" style={{ background:"#fff7ed", border:"1px solid #fed7aa", borderRadius:12, padding:16, marginBottom:16 }}>
            <div style={{ fontWeight:600, fontSize:14, color:"#c2410c" }}>Search cancelled</div>
            <div style={{ fontSize:13, color:"#9a3412", marginTop:4 }}>Hit search again whenever you're ready.</div>
          </div>
        )}
        {status==="error" && (
          <div role="alert" style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:12, padding:16, marginBottom:16 }}>
            <div style={{ fontWeight:600, fontSize:14, color:"#dc2626" }}>⚠ Something went wrong</div>
            <div style={{ fontSize:13, color:"#991b1b", marginTop:4, lineHeight:1.6 }}>{error}</div>
          </div>
        )}
        ...
        {status==="done" && result && (
          <div className="fadeup">
            {rec && (
              <div style={{ background:rec.bg, border:`1.5px solid ${rec.border}`, borderRadius:14, padding:"18px 20px", marginBottom:16, boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                  ...
                  <span style={{ fontWeight:800, fontSize:17, color:rec.color }}>{rec.label}</span>
                  <span style={{ marginLeft:"auto", fontSize:12, color:"#6b7280" }}>{lastRun?.toLocaleDateString("en-GB",{day:"numeric",month:"short"})} · {lastRun?.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}</span>
                </div>
                <div style={{ fontSize:14, color:"#374151", lineHeight:1.7 }}>{result.recommendationReason}</div>
              </div>
            )}
            ...
            <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:12, padding:"14px 16px", marginBottom:16, boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
              <div style={{ fontSize:14, color:"#374151", lineHeight:1.7 }}>{result.summary}</div>
              {result.ofgemAlert && (
                <div style={{ marginTop:12, paddingTop:12, borderTop:"1px solid #f3f4f6", fontSize:13, color:"#b45309", display:"flex", gap:8 }}>
                  <span>📢</span>{result.ofgemAlert}
                </div>
              )}
            </div>

            <div role="tablist" aria-label="Results view" style={{ display:"flex", gap:2, marginBottom:16, background:"#f1f5f9", borderRadius:10, padding:3 }}>
              {[{key:"deals",label:`Deals (${result.deals?.length??0})`},{key:"market",label:"Market overview"}].map(({key,label})=>(
                <button key={key} role="tab" aria-selected={activeTab===key} onClick={()=>setActiveTab(key)} style={{ flex:1, padding:"7px 10px", background:activeTab===key?"#fff":"transparent", border:"none", borderRadius:7, color:activeTab===key?"#111827":"#6b7280", fontSize:13, fontWeight:activeTab===key?700:400, cursor:"pointer", fontFamily:"'Inter',sans-serif", boxShadow:activeTab===key?"0 1px 4px rgba(0,0,0,0.08)":"none", transition:"all 0.15s" }}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div style={{ marginTop:40, paddingTop:24, borderTop:"1px solid #e5e7eb", fontSize:13, color:"#6b7280", lineHeight:1.8, textAlign:"center" }}>
          Results are based on publicly available data. Always verify with a personalised quote before switching.
          <div style={{ marginTop:8, fontSize:12, color:"#6b7280" }}>v{process.env.NEXT_PUBLIC_APP_VERSION}</div>
        </div>
      </main>
    </div>
  );
